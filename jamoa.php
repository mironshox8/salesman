...
?>
 <?php if ($result && $result->num_rows > 0): ?>
                ...
                <?php $n++; endwhile; ?>
            <?php endif; ?><?= number_format($jami_ombor_massa, 2) ?>
...
// SQL so‘rovi
$sql = "SELECT id,mahsulot_nomi, unical_id FROM mahsulotlar";
$result = $conn->query($sql);
?>
...
<body>
    <h2 style="text-align: center;"><b>LOYIHA USTIDA ISHLAGANLAR</b></h2>
    <div class="container">
        <?php
        if ($result->num_rows > 0) {
            while ($row = $result->fetch_assoc()) {
                ...
                $rasm = $row["id"];
                $image_path = "images/" . strtolower(str_replace(' ', '_', $rasm)) . ".jpg";
                
                // Agar rasm mavjud bo'lmasa, default rasm qo'yish
                if (!file_exists($image_path)) {
                    $image_path = "images/default.jpg";
                }
                $bosh = ucfirst(strtolower($mahsulot_nomi));
                echo "<div class='product'>";
                echo "<img src='$image_path' alt='$rasm' width='100' height='120'> <br>";
                echo "<br>";
                echo "<h3><strong>  $mahsulot_nomi </strong></h3>";
                echo "<h4>$unical_id</h4>";
                echo "</div>";
            }
        } else {
